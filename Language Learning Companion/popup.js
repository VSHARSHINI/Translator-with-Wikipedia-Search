...
const _searchWikiLink = document.getElementById('search-wiki');
_searchWikiLink.innerText = i18n.getMessage('searchWiki');
...
function makeSearchWiki(sourceText) {
  let url = 'https://en.wikipedia.org/wiki/Special:Search';
  url += `?search=${encodeURIComponent(sourceText)}`;
  return url;
}
...
async function updateSearchWikiLink() {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!/^https?:\/\//.test(tab?.url)) {
    return;
  }

  _searchWikiLink.href = makeSearchWiki(tab.url, 'auto');
  _searchWikiLink.classList.add('available');
}
