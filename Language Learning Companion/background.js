...
chrome.contextMenus.create({
  id: 'translator_Wikipedia_window',
  contexts: ['action'],
...
chrome.contextMenus.onClicked.addListener(info => {
  switch (info.menuItemId) {
    case 'translator_Wikipedia_window': {
...
async function searchWiki() {
  ...
  chrome.tabs.create({
    openerTabId: tab.id,
    index: tab.index + 1,
    url: makeSearchWiki(selectedText),
  });
}
...
function makeSearchWiki(sourceText) {
  let url = `https://en.wikipedia.org/wiki/Special:Search?search=?search=`;
